...
def getBlobStats(dom, img, arr, boxarr):
    ...
    for b in arr:
        ...
        outputArea.append(Float64(area))
        outputCentroid.append(cent)
        outputColor.append(colo)
    for b in boxarr:
        cv2.drawContours(Bimg,[b],0,(255,255,255),4)
...
def process(img):
    ...
    time4 = time.time()
    areas, centroids, cols, modimag = getBlobStats(img, combAll, Arr, BoxArr)
    ...
    rospy.loginfo(time4-time0)
    rospy.loginfo(time5 - time4)
